...
class Sensors():
    ...
    def __init__(self, n_sectors, sector_radius):
        ...
        self.types_detect = [0, 1]  # types to detect (1=particles, -1=agent, 0=walls)
    ...
    def sense(self, env, force_sense_custom_state=False, custom_state=None):
        ...
        if force_sense_custom_state and custom_state is not None:
            xy_agent = custom_state[0, [0, 1]]  # agent xy
            xy_particles = np.delete(custom_state[:, [0, 1]], [0], axis=0)  # background particles xy
            type_particles = np.delete(custom_state[:, 4], [0], axis=0)  # background particle type
        else:
            xy_agent = env.state[0, [0, 1]]  # agent xy
            xy_particles = np.delete(env.state[:, [0, 1]], [0], axis=0)  # background particles xy
            type_particles = np.delete(env.state[:, 4], [0], axis=0)  # background particle type
        ...
        if self.append_vxy_xy[0] == True:
            ob.append(env.state[0, 2])  # append agent vx
        if self.append_vxy_xy[1] == True:
            ob.append(env.state[0, 3])  # append agent vy
        if self.append_vxy_xy[2] == True:
            ob.append(env.state[0, 0])  # append agent x
        if self.append_vxy_xy[3] == True:
            ob.append(env.state[0, 1])  # append agent y
